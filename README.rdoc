...
Then, continue to implement the features one by one, starting with whatever you deem to be most appropriate. Your final solution should be completely pushed to your private repository by Nov 6, 2015, 11:59pm CET. Make sure that all local changes are committed (git status) and pushed to the upstream repository (i.e., the one on GitHub). 
